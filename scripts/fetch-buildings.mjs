#!/usr/bin/env node
// Descarga las capas vectoriales `builtUp` (Grading) de EMSR916 y agrega el
...
async function getJSON(url) {
  const res = await fetch(url, { headers: { "User-Agent": "terremotocolombia-microsite" } });
...
async function main() {
  ...
  if (!activation) throw new Error("EMSR916 no encontrado");
