#!/usr/bin/env node
// Extrae y normaliza la activación Copernicus EMS Rapid Mapping EMSR916
// (Terremoto de Colombia, 24-jun-2026) desde el backend público del viewer.
//
...
//
// Uso:  node scripts/fetch-emsr916.mjs
// Salida: data/emsr916.json  (normalizado, listo para el microsite)
...
const OUT = resolve(__dir, "../data/emsr916.json");
...
async function main() {
  const res = await fetch(ENDPOINT, {
    headers: { Accept: "application/json", "User-Agent": "terremotocolombia-microsite" },
  });
  if (!res.ok) throw new Error(`HTTP ${res.status} al pedir ${ENDPOINT}`);
  const json = await res.json();
  const activation = json.results?.[0];
  if (!activation) throw new Error("EMSR916 no encontrado en la respuesta");
